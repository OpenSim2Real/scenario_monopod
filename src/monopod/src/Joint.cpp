...
Joint::Joint() : pImpl{std::make_unique<Impl>()} {}
...
size_t Joint::dofs() const { return 1; }
...
bool Joint::setControlMode(const scenario::core::JointControlMode mode) {
  ...
  case core::JointControlMode::Force: {
    ...
    throw std::invalid_argument(
        "Joint '" + this->name() +
        "' is not controllable. Does not support Force control mode.");
  }
...
std::vector<double> Joint::jointPosition() const {
  ...
  throw std::runtime_error("No joint position found for joint '" +
                           this->name() + "'.");
  // return jointPosition;
}
...
std::vector<double> Joint::jointVelocity() const {
  ...
  throw std::runtime_error("No joint velocity found for joint '" +
                           this->name() + "'.");
  // return jointVelocity;
}
...
std::vector<double> Joint::jointAcceleration() const {
  ...
  if (auto data =
          pImpl->monopod_sdk->get_acceleration(pImpl->monopodSdkIndex)) {
  ...
  throw std::runtime_error("No joint acceleration found for joint '" +
                           this->name() + "'.");
  // return jointAcceleration;
}
...
std::vector<double> Joint::jointGeneralizedForceTarget() const {
  ...
  if (this->controlMode() != core::JointControlMode::Force) {
    // LOG(ERROR) << "Joint, '" + this->name() + "' is not in force control
    // mode.";
    throw std::runtime_error("Joint, '" + this->name() +
                             "' is not in force control mode.");
    // return torqueTarget;
  }

  if (auto data_op =
          pImpl->monopod_sdk->get_torque_target(pImpl->monopodSdkIndex)) {
    torqueTarget.push_back(data_op.value());
    return torqueTarget;
  }

  throw std::runtime_error("No generalized force target found for joint '" +
                           this->name() + "'.");
  // return torqueTarget;
}
...
bool Joint::setJointGeneralizedForceTarget(const std::vector<double> &force) {
  ...
  if (force.size() != this->dofs()) {
    // LOG(ERROR) << "Wrong number of elements (joint_dofs=" +
    //                   std::to_string(this->dofs()) + ")";
    throw std::invalid_argument("Wrong number of elements (joint_dofs=" +
                                std::to_string(this->dofs()) + ")");
    // return false;
  }
...
std::vector<double> Joint::jointMaxGeneralizedForce() const {
  ...
  if (auto data_op =
          pImpl->monopod_sdk->get_max_torque_target(pImpl->monopodSdkIndex)) {
    maxGeneralizedForce.push_back(data_op.value());
    // Set to default value here.
    return maxGeneralizedForce;
  }

  throw std::runtime_error("No max generalized force found for joint '" +
                           this->name() + "'.");
  // return maxGeneralizedForce;
}
...
bool Joint::setJointMaxGeneralizedForce(const std::vector<double> &maxForce) {
  ...
  return pImpl->monopod_sdk->set_max_torque_target(maxForce[0],
                                                   pImpl->monopodSdkIndex);
}
...
scenario::core::PID Joint::pid() const {
  if (this->controlMode() != core::JointControlMode::Force) {
    ...
    //                   "' is not in force control mode. Does not support
    //                   PID.";
    throw std::runtime_error(
        "Joint, '" + this->name() +
        "' is not in force control mode. Does not support PID.");
  }
...
bool Joint::setPID(const scenario::core::PID &pid) {
  ...
  return pImpl->monopod_sdk->set_pid(pid.p, pid.i, pid.d,
                                     pImpl->monopodSdkIndex);
}

scenario::core::JointLimit Joint::jointPositionLimit() const {
  if (auto data_op = pImpl->monopod_sdk->get_joint_position_limit(
          pImpl->monopodSdkIndex)) {
    auto data = data_op.value();
    return scenario::core::JointLimit({data.min}, {data.max});
  }

  throw std::runtime_error("No position limit found for joint '" +
                           this->name() + "'.");
  // return scenario::core::JointLimit(this->dofs());
}

scenario::core::JointLimit Joint::jointVelocityLimit() const {
  if (auto data_op = pImpl->monopod_sdk->get_joint_velocity_limit(
          pImpl->monopodSdkIndex)) {
    auto data = data_op.value();
    return scenario::core::JointLimit({data.min}, {data.max});
  }

  throw std::runtime_error("No velocity limit found for joint '" +
                           this->name() + "'.");
  // return scenario::core::JointLimit(this->dofs());
}

scenario::core::JointLimit Joint::jointAccelerationLimit() const {
  if (auto data_op = pImpl->monopod_sdk->get_joint_acceleration_limit(
          pImpl->monopodSdkIndex)) {
    auto data = data_op.value();
    return scenario::core::JointLimit({data.min}, {data.max});
  }

  throw std::runtime_error("No acceleration limit found for joint '" +
                           this->name() + "'.");
  // return scenario::core::JointLimit(this->dofs());
}

bool Joint::setJointPositionLimit(const double &maxPosition,
                                  const double &minPosition) {
  return pImpl->monopod_sdk->set_joint_position_limit(maxPosition, minPosition,
                                                      pImpl->monopodSdkIndex);
}

bool Joint::setJointVelocityLimit(const double &maxVelocity,
                                  const double &minVelocity) {
  return pImpl->monopod_sdk->set_joint_velocity_limit(maxVelocity, minVelocity,
                                                      pImpl->monopodSdkIndex);
}

bool Joint::setJointAccelerationLimit(const double &maxAcceleration,
                                      const double &minAcceleration) {
  return pImpl->monopod_sdk->set_joint_acceleration_limit(
      maxAcceleration, minAcceleration, pImpl->monopodSdkIndex);
}
